...
probGrid.plot <- function(ppi_prediction_result, n=100, x.log.scale = TRUE, xlim = c(0,NA), ylim = NULL, set="train", model="all") {
  assertthat::assert_that(set %in% c("test", "train"), msg = "'set=' must be 'test' or 'train'.")
  assertthat::assert_that(model == "all" | is.integer(model) & model %in% seq_along(ppi_prediction_result$model.e))
  if (set == "test") {
  ...
  if (length(ppi_prediction_result$assay) == 1 & model == "all") {
  ...
  } else if (length(ppi_prediction_result$assay) >= 2 & model == "all") {
    p <- ggplot2::ggplot() + ggplot2::geom_point(data = xgrid %>%
                                                   base::cbind(ygrid) %>% base::cbind(ygrid.prob) %>%
  ...
  if (length(ppi_prediction_result$assay) == 1 & model %in% seq_along(ppi_prediction_result$model.e)) {
    p <- ggplot2::ggplot() + ggplot2::geom_point(data = xgrid %>%
                                                   base::cbind(ygrid) %>% base::cbind(ygrid.prob) %>%
                                                   base::as.data.frame(), mapping = ggplot2::aes(x = id,
                                                                                                 y = !!rlang::sym(ppi_prediction_result$assay[1]),
      ...
      ggplot2::theme(text = element_text(family = "Avenir"),
                     ...
                     axis.text.x = element_text(angle = 90, vjust = 0.5,
                                                hjust = 1), legend.position = "right")
  } else if (length(ppi_prediction_result$assay) >= 2 & model %in% seq_along(ppi_prediction_result$model.e)) {
    p <- ggplot2::ggplot() + ggplot2::geom_point(data = xgrid %>%
                                                   base::cbind(ygrid) %>% base::cbind(ygrid.prob) %>%
                                                   base::as.data.frame(), mapping = ggplot2::aes(x = !!rlang::sym(ppi_prediction_result$assay[2]),
                                                                                                 y = !!rlang::sym(ppi_prediction_result$assay[1]),
