...
probGrid.plot <- function(ppi_prediction_result, n=100, x.log.scale = TRUE, xlim = NULL, ylim = NULL) {
  ...
  make.grid <- function(ppi_prediction_result) {
    ...
    if (length(ppi_prediction_result$assay) == 1) {
      x <- base::cbind(ppi_prediction_result$testMat,
                       row_number = 1:length(ppi_prediction_result$testMat))
      x.log.scale <- FALSE
  ...
  if (x.log.scale == TRUE) {
    min.x <- min(ppi_prediction_result$predDf %>% dplyr::select(!!rlang::sym(ppi_prediction_result$assay[2])), na.rm = TRUE)
    if(min.x < 0) {
  ...
  if (length(ppi_prediction_result$assay) == 1) {
    p <- ggplot2::ggplot() + ggplot2::geom_point(data = xgrid %>%
      ...
      ggnewscale::new_scale("fill") + ggplot2::geom_point(data = ppi_prediction_result$predDf %>%
