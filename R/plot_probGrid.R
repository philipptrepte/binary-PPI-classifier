...
#' @param y.log.scale: logical to log-scale y-axis values
...
#' @param y.nudge: Numerical. Which value to add to log transformation of y-axis values, in case of negative y values.
...
probGrid.plot <- function(ppi_prediction_result, n=100, x.log.scale = FALSE, y.log.scale = FALSE, xlim = c(NA,NA), ylim = c(NA,NA), set="train", model="all", training_set = 'all', x.nudge = 1, y.nudge = 1, type = "2D", assay = ppi_prediction_result$assay) {
  ...
  make.grid <- function(ppi_prediction_result) {
    ...
    if(y.log.scale == TRUE) {
      if (any(grange < 0)) {
        grangeY <- base::apply(grange, 2, function(x) x + abs(min(x, na.rm = TRUE)) + y.nudge)[,1]
      }
      x1 <- lseq(from = grangeY[1], to = grangeY[2], length = n)
    } else if (y.log.scale == FALSE) {
      x1 <- base::seq(from = grange[1, 1], to = grange[2, 1], length = n)

    }
    if (x.log.scale == TRUE) {
      if (any(grange < 0)) {
        grangeX <- base::apply(grange, 2, function(x) x + abs(min(x)) + x.nudge)[,2]
      }
      x2 <- lseq(from = grangeX[1], to = grangeX[2], length = n)
  ...
  if(y.log.scale == TRUE & x.log.scale == TRUE) {
    xgrid_new <- xgrid %>% dplyr::mutate(!!(rlang::sym(a)) := !!(rlang::sym(a)) - abs(min(Df[,a], na.rm = TRUE) - y.nudge),
                                         !!(rlang::sym(b)) := !!(rlang::sym(b)) - abs(min(Df[,b], na.rm = TRUE) - y.nudge))
  } else if(y.log.scale == TRUE & x.log.scale == FALSE) {
    xgrid_new <- xgrid %>% dplyr::mutate(!!(rlang::sym(a)) := !!(rlang::sym(a)) - abs(min(Df[,a], na.rm = TRUE) - y.nudge))
  } else if(x.log.scale == TRUE & y.log.scale == FALSE) {
    xgrid_new <- xgrid %>% dplyr::mutate(!!(rlang::sym(b)) := !!(rlang::sym(b)) - abs(min(Df[,b], na.rm = TRUE) - y.nudge))
  } else if(x.log.scale == FALSE & y.log.scale == FALSE) {
    xgrid_new <- xgrid
  }
  ...
  if (model == "all") {
    for (i in base::seq(1, length(ppi_prediction_result$model.e))) {
      if (i == 1) {
        ygrid <- base::data.frame(base::matrix(nrow = base::nrow(xgrid_new),
                                               ncol = 0))
        ygrid.prob <- base::data.frame(base::matrix(nrow = base::nrow(xgrid_new),
                                                    ncol = 0))
      }
      ygrid = base::cbind(ygrid, data.frame(stats::predict(ppi_prediction_result$model.e[[i]], xgrid_new)))
      ygrid.prob = base::cbind(ygrid.prob, attr(stats::predict(ppi_prediction_result$model.e[[i]], xgrid_new, probability = TRUE), "probabilities")[, 1])
  ...
  if (model %in% seq_along(ppi_prediction_result$model.e)) {
    ygrid <- base::data.frame(base::matrix(nrow = base::nrow(xgrid_new),ncol = 0))
    ygrid.prob <- base::data.frame(base::matrix(nrow = base::nrow(xgrid_new),ncol = 0))
    ygrid = data.frame(ygrid = stats::predict(ppi_prediction_result$model.e[[model]], xgrid_new))
    ygrid.prob = base::cbind(ygrid.prob, attr(stats::predict(ppi_prediction_result$model.e[[model]],
                                                             xgrid_new, probability = TRUE), "probabilities")[, 1])
  }
  if (y.log.scale == TRUE) {
    xgrid[,a] <- log(xgrid[,a])
    min.y <- min(Df[,a], na.rm = TRUE)
    if (min.y < 0) {
      Df[,a] <- log(Df[,a] + abs(min.y) + y.nudge)
    } else {
      Df[,a] <- log(Df[,a])
    }
  }
  if (x.log.scale == TRUE) {
    xgrid[,b] <- log(xgrid[,b])
    min.x <- min(Df[,b], na.rm = TRUE)
    if (min.x < 0) {
      Df[,b] <- log(Df[,b] + abs(min.x) + x.nudge)
    } else {
      Df[,b] <- log(Df[,b])
    }
  }
  if (length(ppi_prediction_result$assay) == 1 & model == "all" & type == "2D") {
    p <- ggplot2::ggplot() + ggplot2::geom_point(data = xgrid %>%
      ...
      ggplot2::geom_point(data = Df %>%
                          ...
                          mapping = ggplot2::aes(x = id, y = !!rlang::sym(ppi_prediction_result$assay[1]), size = predMat, shape = reference, col = reference),
