...
probDis.plot <- function(ppi_prediction_result, set="test") {
  ...
  if(set == "test") {
    plot <- ppi_prediction_result$predDf %>%
      ggplot2::ggplot(aes(x = !!rlang::sym(ppi_prediction_result$assay[1]), y = predMat, fill = complex)) +
      ...
      ggplot2::labs(x = ppi_prediction_result$assay[1], y = "probability") +
    ...
    plot <- ppi_prediction_result$predTrainDf %>%
      ggplot2::ggplot(aes(x = !!rlang::sym(ppi_prediction_result$assay[1]), y = predTrainMat, fill = reference)) +
      ggplot2::geom_point(shape = 21, alpha = 0.7, size = 1.5) +
      ggpubr::theme_pubr() +
      ggplot2::theme(legend.position = "none") +
